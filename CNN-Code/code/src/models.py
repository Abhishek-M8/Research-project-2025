    ...
    def load(self):
        try:
            if os.path.exists(self.weights):
                if self.weights.find('pw') != -1:  # Fixed: use != instead of 'is not'
                    print('Loading "patch-wise" model...')
                elif self.weights.find('iw') != -1:  # Fixed: use != instead of 'is not'
                    print('Loading "image-wise" model...')
                self.network.load_state_dict(torch.load(self.weights))
        except Exception as e:
            print('Failed to load pre-trained network from ', self.weights)
...
class PatchWiseModel(BaseModel):
    def __init__(self, args, network):
        super(PatchWiseModel, self).__init__(args, network, args.checkpoints_path + '/weights_pw1.pth')

    def train(self):
        self.network.train()
        print('Start training patch-wise network: {}\n'.format(time.strftime('%Y/%m/%d %H:%M')))

        trn_path = os.path.join(self.args.dataset_path, TRAIN_PATH)
        train_loader = DataLoader(
            dataset=PatchWiseDataset(path=trn_path, stride=self.args.patch_stride, rotate=True, flip=True, enhance=True),
    ...
    def validate(self, verbose=True):
        ...
        val_path = os.path.join(self.args.dataset_path, VALIDATION_PATH)
        test_loader = DataLoader(
            dataset=PatchWiseDataset(path=val_path, stride=self.args.patch_stride),
            batch_size=self.args.batch_size,
            shuffle=False,
            num_workers=12
        )
        ...
        for images, labels in test_loader:
            ...
            test_loss += F.nll_loss(output, Variable(labels), reduction='sum').data.item()
            _, predicted = torch.max(output.data, 1)
            correct += torch.sum(predicted == labels)
...
class ImageWiseModel(BaseModel):
    def __init__(self, args, image_wise_network, patch_wise_network):
        super(ImageWiseModel, self).__init__(args, image_wise_network, args.checkpoints_path + '/weights_iw_h2gnew_trn1.pth')

        self.patch_wise_model = PatchWiseModel(args, patch_wise_network)
        self._test_loader = None # self._patch_loader(self.args.dataset_path + TRAIN_PATH, False)

    def train(self):
        self.network.train()
        print('Evaluating patch-wise model...')
        trn_path = os.path.join(self.args.dataset_path, TRAIN_PATH)
        train_loader = self._patch_loader(trn_path, True)
    ...
    def validate(self, verbose=True, roc=False):
        ...
        if self._test_loader is None:
            val_path = os.path.join(self.args.dataset_path, VALIDATION_PATH)
            self._test_loader = self._patch_loader(val_path, False)
        ...
        labels_true = []
        labels_pred = np.empty((0, classes))
        ...
        for images, labels in self._test_loader:
            ...
            val_loss += F.nll_loss(output, Variable(labels), reduction='sum').data.item()
            _, predicted = torch.max(output.data, 1)
            correct += torch.sum(predicted == labels)
    ...
    # MODIFIED FUNCTION: Added return_probs parameter and probability collection
    def test(self, path, verbose=True, ensemble=True, return_probs=False):
        self.network.eval()
        ...
        res = []
        all_probs = []  # NEW: Store probabilities for each tile
        ...
        for index, (image, file_name) in enumerate(data_loader):
            ...
            # NEW: Collect probabilities before processing
            probs = torch.exp(output).cpu().detach().numpy()  # Convert to probabilities
            all_probs.append(probs)  # Store probabilities for this tile

            maj_prob = 5 - np.argmax(np.sum(np.eye(6)[np.array(predicted.cpu().numpy()).reshape(-1)], axis=0)[::-1])

            confidence = np.sum(np.array(predicted.cpu().numpy()) == maj_prob) / n_bins if ensemble else torch.max(torch.exp(output.data))
        ...
        # NEW: Return probabilities if requested
        if return_probs:
    ...
    def _patch_loader(self, path, augment):
        ...
        if self.args.debug and augment and os.path.exists(images_path):
            ...
            bsize = 8
            output_loader = DataLoader(dataset=dataset, batch_size=bsize, shuffle=True, num_workers=14)
            output_images = []
            output_labels = []
