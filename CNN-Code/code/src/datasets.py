...
LABELS = ['CBT', 'CBTA', 'CBTP', 'CBT3', 'CBTPA', 'CBTP3']
...
class PatchWiseDataset(Dataset):
    def __init__(self, path, stride=PATCH_SIZE, rotate=False, flip=False, enhance=False):
        ...
        print(path)
        labels = {name: index for index in range(len(LABELS)) for name in glob.glob(path + '/' + LABELS[index] + '/*.png')}
...
#         print(path)
        ...
        labels = {name: index 
                  ...
                  for name in glob.glob(path + '/' + LABELS[index] + '/*.png')}
#         print(glob.glob(path + '/' + LABELS[index] + '/*.png'))
